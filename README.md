...
## Phase 3b — App auth (foundation)

Phase 3b-1 adds **self-hosted BetterAuth** (roles `trainer` | `client`) so the
app becomes a real two-audience product. It **retires the
`EXPO_PUBLIC_TRAINER_ID` stop-gap**: trainer context now comes from the
authenticated session. This checkpoint ships the auth *foundation* — the
client dashboard and trainer lead/booking management land in 3b-2.

### What it adds

- **API** — `apps/api/src/lib/auth.ts` (BetterAuth + Drizzle adapter; `expo()` +
  `bearer()` plugins; `role` / `trainerId` / `clientId` carried on the BetterAuth
  `user`), mounted at `/api/auth/*` in `app.ts`. A global session middleware
  populates `c.var.user` from a cookie (web) **or** bearer token (native). Role
  guards (`apps/api/src/middleware/auth.ts`) now protect the Phase 2 trainer
  routes; `/trainers/:trainerId/*` additionally requires the path id to match the
  session trainer (→ `403` otherwise). CORS is tightened from allow-all to a
  **credentialed origin allow-list**.
- **Auth tables via a committed migration** — `drizzle/0001_betterauth_tables.sql`
  creates `user`, `auth_session` (renamed from `session` to avoid colliding with
  the domain training-`session` table), `account`, `verification`. No hand-written
  SQL: the Drizzle schema is `apps/api/src/db/auth-schema.ts` and the SQL is
  generated by `drizzle-kit`.
- **Mobile** — `lib/auth-client.ts` (BetterAuth Expo client, secure-storage on
  native / cookies on web), a Design-System `/login` screen, an auth **guard** on
  the `(app)` route group (unauthenticated → `/login`; role decides the landing:
  trainer → `/dogs`, client → `/client`), and a `/client` landing placeholder.

### New environment (`.env`)

| Var | Required? | Purpose |
| --- | --- | --- |
| `AUTH_SECRET` | **Yes** (throws on startup if unset) | Signs BetterAuth sessions. Generate: `openssl rand -base64 32`. |
| `BETTER_AUTH_URL` | No (defaults to `http://localhost:<PORT>`) | Auth base URL for the `/api/auth/*` endpoints. Set to the public API origin in prod. |
| `ALLOWED_ORIGINS` | No (defaults to the Expo web dev origins) | Comma-separated credentialed CORS allow-list. |

### Seed + run + log in (web)

```bash
# 1. Apply migrations (creates the 11 domain tables + the 4 auth tables)
npm run db:migrate -w apps/api

# 2. Seed the demo graph + BOTH auth logins (trainer + client)
npm run db:seed -w apps/api
#   → trainer@tailsup.local / Trainer123!   (role trainer, linked to the practice)
#   → client@tailsup.local  / Client123!    (role client, linked to a dog + homework)

# 3. Run the API + the app (web)
npm run dev -w apps/api
npm run web -w apps/mobile
```

Open the web app, go to **/login**, and sign in:

- **trainer@tailsup.local** → lands on **/dogs** (their dogs, resolved from the
  session — no `EXPO_PUBLIC_TRAINER_ID`). "Sign out" returns to `/login`.
- **client@tailsup.local** → lands on **/client** (dashboard placeholder for 3b-1).
- Visiting an `(app)` route while signed out redirects to **/login**; the public
  site routes stay open.

> **Dev caveat (this machine):** `node`/`npm` default to Node 16 in fresh shells.
> `nvm use 20.20.2` repoints the symlink but flaps mid-command, which hangs
> `vitest` worker spawn. Run tooling with the **absolute** Node 20 binary, e.g.
> `PATH="$HOME/AppData/Roaming/nvm/v20.20.2:$PATH" node …/vitest/vitest.mjs run`.

### Verify

- `npm run typecheck --workspaces` → clean (API + mobile).
- API tests: **186 pass** (13 new auth-guard tests; the Phase 2 route tests now
  carry a mocked trainer session — see `apps/api/src/test/authMock.ts`).
